...
-- Note: With header-based auth, we'll rely on application-level checks
-- This policy allows updates, but app will verify ownership
...
-- Bets are immutable (no updates/deletes allowed)
-- Only system can modify during resolution

-- Note: Since we're using header-based authentication (x-user-id) instead of Supabase Auth JWT tokens,
-- RLS policies using auth.uid() won't work. These policies provide a basic security layer,
-- but authorization (who can do what) is primarily handled in the application layer through middleware.
-- 
